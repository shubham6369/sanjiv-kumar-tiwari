    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            resultDiv.innerHTML = `
                ...
                ${data.statusDocUrl ? `<div style="margin-top:10px;"><a href="${data.statusDocUrl.replace('/upload/', '/upload/fl_attachment:false/')}" target="_blank" style="display:inline-block; background:#0b5c3b; color:white; padding:8px 15px; border-radius:6px; text-decoration:none; font-weight:600;"><i class="fas fa-file-alt"></i> View Status Document</a></div>` : ''}
                ${data.photoUrl ? (data.photoUrl.toLowerCase().includes('.pdf') ? `<div style="margin-top:10px;"><strong>डॉक्यूमेंट (PDF):</strong><br><a href="${data.photoUrl.replace('/upload/', '/upload/fl_attachment:false/')}" target="_blank" style="display:inline-block; background:#1565c0; color:white; padding:8px 15px; border-radius:6px; text-decoration:none; font-weight:600; margin-top:5px;"><i class="fas fa-file-pdf"></i> View Submitted PDF</a></div>` : `<div style="margin-top:10px;"><strong>फोटो:</strong><br><a href="${data.photoUrl.replace('/upload/', '/upload/fl_attachment:false/')}" target="_blank"><img src="${data.photoUrl.replace('/upload/', '/upload/fl_attachment:false/')}" style="max-width:100%; border-radius:8px; margin-top:5px; border:1px solid #ddd; max-height:200px; object-fit:contain;"></a></div>`) : ''}`;
        } else {
